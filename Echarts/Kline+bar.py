...
#date = pd.to_datetime(df.index).strftime('%Y/%m/%d').tolist()
price = df[['open','close','low','high']].values.tolist()
close = df['close']
volume = df['amount'].values.tolist()
...
bar=(
    ...
    .add_yaxis(
        series_name='Volume',
        y_axis=[[i,volume[i],1 if price[i][0]>price[i][1] else -1]
...
#kline.overlap(line).render("K线图.html")
...
grid_chart.render("Kline+bar.html")
